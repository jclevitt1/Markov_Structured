...
class CzechStateDeterminer(StateDeterminer):
    ...
    def assign_state_to_vals(self, val, std):
        if 0 <= val < std * self.k:
            return 'g1'
        elif std * self.k <= val < 2 * std * self.k:
            return 'g2'
        elif 2 * std * self.k <= val < 3 * std * self.k:
            return 'g3'
        elif 3 * std * self.k <= val:
            return 'g4'
        elif -1 * std * self.k <= val < 0:
            return 'd1'
        elif -2 * std * self.k <= val < -1 * std * self.k:
            return 'd2'
        elif -3 * std * self.k <= val < -2 * std * self.k:
            return 'd3'
        else:
            return 'd4'
    ...
    def assign_state_to_data(self, data):
        ...
        for i in range(self.l, len(data)):
            ...
            state = self.assign_state_to_vals(data.iloc[i][self.col_to_std], std)
